...
export default function RegisterPage() {
  ...
  async function handleRegister() {
    ...
    if (event.is_paid && !phone) { setError("Phone number required for M-Pesa payment"); return; }
    setSubmitting(true);
    setError("");
    ...
    const { data: reg, error: regError } = await supabase.from("registrations").insert({
      ...
      guest_access_link: `${window.location.origin}/e/${eventSlug}/g/${accessToken}`,
    }).select().single();

    if (regError) { setError(regError.message); setSubmitting(false); return; }

    setGuestLink(`${window.location.origin}/e/${eventSlug}/g/${accessToken}`);
    setSuccess(true);
    setSubmitting(false);
  }
  ...
  if (success) return (
    <div style={{minHeight:"100vh",background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      ...
      <h1 style={{fontSize:"24px",fontWeight:"300",color:"#fff",textAlign:"center",marginBottom:"8px"}}>You're in.</h1>
      ...
      <p style={{color:"#444",fontSize:"13px",textAlign:"center"}}>Save this link — it's your ticket and networking pass.</p>
    </div>
  );
