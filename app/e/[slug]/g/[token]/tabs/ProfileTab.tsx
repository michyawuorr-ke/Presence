...
export default function ProfileTab({profile,event,onProfileUpdate,isEnded,registration}:ProfileTabProps){
  ...
  useEffect(()=>{
    ...
    supabase.from("connection_notes").select("about_id,note").eq("author_id",profile.id).then(({data})=>{
      if(data){const map:Record<string,string>={};data.forEach((n:any)=>{if(n.note)map[n.about_id]=n.note;});setSavedNotes(map);}
    });
  },[profile]);
  ...
  useEffect(()=>{
    ...
    async function loadIncomingSignals(){
      ...
      if(cancelled)return;
      setIncomingSignals(signals.map((s:any)=>({...s,senderName:(senders||[]).find((p:any)=>p.id===s.sender_id)?.display_name||"Someone",locationLabel:s.custom_location||(stations||[]).find((st:any)=>st.id===s.station_id)?.name||"a meetup spot"})));
    }
    loadIncomingSignals();
    const ch=supabase.channel("incoming-signals:"+profile.id).on("postgres_changes",{event:"INSERT",schema:"public",table:"meetup_signals"},(payload:any)=>{if(payload.new.recipient_id===profile.id)loadIncomingSignals();}).subscribe();
  ...
  async function sendSignalMeetup(){
    ...
    const{error}=await supabase.from("meetup_signals").insert({event_id:event.id,sender_id:profile.id,recipient_id:signalTarget.id,station_id:signalStationId||null,custom_location:signalStationId?null:signalCustomLocation.trim(),status:"pending"});
    if(!error){setSignalSentIds(prev=>new Set(prev).add(signalTarget.id));setSignalNotification(`Meetup signal sent to ${getFirstName(signalTarget.display_name)}`);setTimeout(()=>setSignalNotification(""),4000);}
  ...
  useEffect(()=>{
    ...
    async function loadPending(){
      ...
      if(cancelled)return;
      setPendingRequests(reqs.map((r:any)=>{const requester=(requesters||[]).find((p:any)=>p.id===r.requester_id);return requester?{requestId:r.id,reason:r.reason,...requester,networking_intents:parseIntents(requester.networking_intents)}:{requestId:r.id};}).filter((r:any)=>r.id&&r.display_name));
    }
    loadPending();
    const ch=supabase.channel("pending-requests:"+profile.id).on("postgres_changes",{event:"INSERT",schema:"public",table:"handshake_requests"},(payload:any)=>{if(payload.new.recipient_id===profile.id)loadPending();}).subscribe();
    return()=>{cancelled=true;supabase.removeChannel(ch);};
  },[profile,event]);
  ...
  async function respondToPending(requestId:string,requesterId:string,approve:boolean){
    ...
    if(updateErr){setSignalNotification("❌ Couldn't update request: "+updateErr.message);setTimeout(()=>setSignalNotification(""),8000);return;}
    if(approve){
      const{error:hsErr}=await supabase.from("handshakes").insert({sender_id:requesterId,receiver_id:profile.id,status:"accepted"});
      if(hsErr){setSignalNotification("❌ Couldn't create connection: "+hsErr.message);setTimeout(()=>setSignalNotification(""),8000);}
    }
  }
  ...
  useEffect(()=>{
    ...
    async function loadConnections(){
      ...
      setConnections((profiles||[]).map((p:any)=>{const hs=(handshakes||[]).find((h:any)=>h.sender_id===p.id||h.receiver_id===p.id);return{...p,handshakeId:hs?.id};}));
    }
    loadConnections();
    const ch=supabase.channel("profile-handshakes:"+profile.id)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"handshakes"},(payload:any)=>{if(payload.new.sender_id===profile.id||payload.new.receiver_id===profile.id)loadConnections();})
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"handshakes"},(payload:any)=>{if(payload.new.sender_id===profile.id||payload.new.receiver_id===profile.id)loadConnections();})
      .subscribe();
    ...
    try{
      ...
      await scanner.start({facingMode:"environment"},{fps:10,qrbox:{width:200,height:200}},async(decoded:string)=>{
        if(decoded.startsWith("presence:unlock:")){
          ...
          if(res.ok){setUnlocked(prev=>new Set([...prev,conn.id]));setScanMsg("✅ Profile unlocked!");setScanTarget(null);setMemoryTarget(conn);setMemoryDraft("");}
          else{const body=await res.json().catch(()=>({}));setScanMsg("❌ "+(body.error||"Could not unlock."));}
          setTimeout(()=>setScanMsg(""),4000);
        }
      },()=>{});
    }catch(err:any){console.error("Scanner error:",err);setScanning(false);setScanMsg("Camera not available — check permissions.");setTimeout(()=>setScanMsg(""),6000);}
  }
  ...
  return(
    <div style={{padding:"16px",background:"#08080a",minHeight:"100vh"}}>
      ...
      <div style={{marginTop:"16px"}}>
        ...
        {connections.length===0?(
          ...
          connections.map((c:any)=>{
            const isUnlocked=unlocked.has(c.id);const signalSent=signalSentIds.has(c.id);const hasNote=!!savedNotes[c.id];
            return(
              <div key={c.id} style={{background:isUnlocked?"rgba(226,109,52,0.08)":"rgba(26,26,36,0.9)",borderRadius:"14px",padding:"14px",marginBottom:"8px",border:isUnlocked?"1px solid rgba(226,109,52,0.25)":"1px solid rgba(255,255,255,0.06)"}}>
                ...
                {isUnlocked&&(
                  <div style={{display:"flex",gap:"8px",marginTop:"10px"}}>
                    ...
                    <button onClick={()=>{setMemoryTarget(c);setMemoryDraft(savedNotes[c.id]||"");}} style={{flexShrink:0,padding:"8px 12px",borderRadius:"8px",background:"transparent",border:"1px solid rgba(255,255,255,0.1)",color:hasNote?"#E26D34":"rgba(240,237,232,0.4)",fontSize:"12px",cursor:"pointer"}}>{hasNote?"📝 Note":"+ Note"}</button>
                  </div>
                )}
              </div>
            );
      ...
      {signalNotification&&<div style={{background:"rgba(226,109,52,0.08)",border:"1px solid rgba(226,109,52,0.2)",borderRadius:"12px",padding:"10px 14px",marginTop:"12px"}}><p style={{color:"#E26D34",fontSize:"12px",margin:0,textAlign:"center"}}>{signalNotification}</p></div>}
      {reportMsg&&<p style={{color:"#666",fontSize:"12px",textAlign:"center",margin:"8px 0"}}>{reportMsg}</p>}
