"use client";
// Shown post-event for connections made via request/accept rather than a
// QR scan. Same choice as DisclosureSheet (what can this person see from
// my profile), but there's no profile_unlocks row to attach to since no
// scan happened — inserts with unlock_id: null instead.
...
export default function AcceptDisclosureSheet({ ownerId, viewerId, viewerName, myProfile, onSave, onSkip }: Props) {
  ...
  ] as DisclosureOption[])).filter(o => o.value?.trim());
  ...
  // Supabase's upsert() can't target a partial unique index (the one on
  // (owner_id, viewer_id) WHERE unlock_id IS NULL) — Postgres requires the
  // ON CONFLICT clause's WHERE to match exactly, which the JS client has no
  // way to express. So this checks for an existing row manually instead.
  async function saveDisclosure(fields: { show_linkedin: boolean; show_website: boolean; show_portfolio: boolean; show_phone: boolean }) {
    const { data: existing } = await supabase
      .from("connection_disclosures")
      .select("id")
      .eq("owner_id", ownerId)
    ...
    if (existing) {
      await supabase.from("connection_disclosures")
        .update({ ...fields, updated_at: new Date().toISOString() })
        .eq("id", existing.id);
    } else {
      await supabase.from("connection_disclosures").insert({
        unlock_id: null,
        owner_id: ownerId,
        viewer_id: viewerId,
        ...fields,
  ...
  async function skip() {
    // Skipping still records a row (all fields false) so the prompt
    // doesn't reappear — matches how "already prompted" is determined
    // in useConnections (any row with unlock_id null counts, regardless
    // of what was chosen).
    setSaving(true);
    await saveDisclosure({
      show_linkedin: false,
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "flex-end", zIndex: 60 }}>
      <div style={{ background: "#0E0E0E", borderRadius: "24px 24px 0 0", padding: "28px 24px", paddingBottom: "calc(28px + env(safe-area-inset-bottom))", width: "100%", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <p style={{ fontSize: "18px", fontWeight: "600", color: "#f0ede8", margin: "0 0 4px" }}>
          Share contact details with {first}?
        </p>
        <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", margin: "0 0 24px", lineHeight: "1.5" }}>
          You connected at this event — choose what {first} can see from your profile. You can always update this later.
        </p>
